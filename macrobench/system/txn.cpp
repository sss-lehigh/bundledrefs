...
void txn_man::init(thread_t *h_thd, workload *h_wl, uint64_t thd_id) {
  ...
  insert_cnt = 0;
  accesses =
      (Access **)_mm_malloc(sizeof(Access *) * MAX_ROW_PER_TXN, ALIGNMENT);
...
int txn_man::index_range_query(INDEX *index, idx_key_t low, idx_key_t high,
  ...
  uint64_t starttime = get_sys_clock();
  int numResults = 0;
  index->index_range_query(low, high, resultKeys, resultValues, &numResults,
                           part_id);
  INC_TMP_STATS(get_thd_id(), stats_indexes[index->index_id].numRangeQuery, 1);
  INC_TMP_STATS(get_thd_id(), stats_indexes[index->index_id].timeRangeQuery,
...
itemid_t *txn_man::index_read(INDEX *index, idx_key_t key, int part_id) {
  ...
  INC_TMP_STATS(get_thd_id(), stats_indexes[index->index_id].timeContains,
                get_sys_clock() - starttime);
  return item;
}
...
void txn_man::index_insert(INDEX *index, uint64_t key, row_t *row,
                           int64_t part_id) {
  uint64_t starttime = get_sys_clock();
  get_wl()->index_insert(index, key, row, part_id);
  INC_TMP_STATS(get_thd_id(), stats_indexes[index->index_id].numInsert, 1);
  INC_TMP_STATS(get_thd_id(), stats_indexes[index->index_id].timeInsert,
                get_sys_clock() - starttime);
}
