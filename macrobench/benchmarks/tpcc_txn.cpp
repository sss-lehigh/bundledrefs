...
+=======================================================*/
  ...
  for (UInt32 ol_number = 0; ol_number < ol_cnt; ol_number++) {
    ...
    assert(item != NULL);  // Ensures that an item is always found.
  ...
  // YYY: We assume at this point that the transaction is doomed to commit, and
  // therefore update the indexes accordingly. Some inconsistencies between the
  // index and the data base may be possible, but we do not expect high abort
  // rates.
  ...
  for (int i = 0; i < bufsize; ++i) {
    ...
    index_insert(_wl->i_orderline_wd, orderline_wdKey(w_id, d_id), buf[i],
                 wh_to_part(w_id));
    // YYY: Insert into new order index.
    index_insert(_wl->i_neworder, neworderKey(w_id, d_id, o_id), buf[i],
                 wh_to_part(w_id));
  }
  // #endif
  return finish(rc);
...
// yet.
// Incorrect states may happen with the current code.

RC tpcc_txn_man::run_delivery(tpcc_query *query) {
  // XXX HACK if another delivery txn is running on this warehouse, simply
  // commit.
  if (!ATOM_CAS(_wl->delivering[query->w_id], false, true)) return finish(RCOK);
  ...
  for (int d_id = 1; d_id <= DIST_PER_WARE; d_id++) {
    ...
    item =
        index_read(_wl->i_order_line, orderlineKey(query->w_id, d_id, no_o_id));
    ...
    itemid_t *item =
        index_read(_wl->i_customer_id, key, wh_to_part(query->w_id));
    row_t *r_cust = (row_t *)item->location;
    double c_balance;
